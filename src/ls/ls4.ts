...

{
  ...
  function getLastPatIndex(value: string, pat: RegExp) {
    if (pat.flags.includes('g')) {
      return Array.from(value.matchAll(pat3))?.pop()?.index;
    }
    throw new TypeError(
      'the global flag is missing from the pattern'
    );
  }
...

{
  ...
  const pat4 =
    /[-+]?\d+((\.\d+(e\d+)?)|e\d+)?,[-+]?\d+((\.\d+(e\d+)?)|e\d+)?/g;

  function isEmpty<T>(value: T): value is NonNullable<T> {
    return value != null;
  }
  console.log(
    Array.from(row1.matchAll(pat4), (item) => {
      return item[0]
        .split(',')
  ...
  console.log(
    Array.from(row2.matchAll(pat4), (item) => {
      return item[0]
        .split(',')
        .reduce((x, y) => x + Number(y), 0);
    })
